...
contentBlocks:
    ...
    body:
      ko: >-
        ...
        flowchart TB
         ...
         S1[Hugging Face, 정체불명의<br/>AI 공격을 탐지·차단]
         end

         P[<b>07.20~22</b><br/>워싱턴의 신호 세 건<br/>• 중국 모델 사용 금지 검토 보도<br/>• 재무장관, 증류에 따른 제재 시사<br/>• 백악관 과기보좌관, Fable 증류 의혹 제기]
         S2[<b>07.21 화</b><br/>OpenAI 공시<br/>그 공격자는 자사 모델]
         R1[<b>07.22 수</b><br/>스타트업 약 200곳<br/>금지 반대 서한]
         R2[<b>07.24 금</b><br/>공개서한 게시 · 25곳]
         W[<b>07.25 주말</b><br/>OpenAI·Google 합류<br/>Anthropic 불참이 두드러짐]

         subgraph D27[07.27 월]
         K1[문샷, K3 가중치 전면 공개]
         A[아모데이 반박문<br/>금지를 주장한 적 없다]
         O[NVIDIA 주도 얼라이언스<br/>OSAA 발족]
         end

         N[<b>07.30 목</b><br/>서명 235곳]
  ...
  - type: text
    body:
      ko: >-
        위에서 아래가 날짜 흐름, 점선은 공개자료에서 확인되거나 내용상 직접 이어지는 관계다. 점선이 없는 사건은 같은 시기에 일어났지만
        직접적인 연결이 확인되지 않았다.
        ...
        **증류**는 이미 학습된 모델의 출력을 교사 삼아 다른 모델을 학습시키는 기법이다. 모델 개발과 평가, 경량화에 널리 사용되는 방법이지만, 미국 정부와 일부 모델 기업은 타사의 상용 모델을 상대로 은밀한 대규모 추출을 벌이는 경우 이를 정상적인 학습 기법이 아니라 지식재산 탈취로 봐야 한다고 주장한다.
        ...
        2015년 설립문에서 OpenAI는 주주의 이익보다 모두를 위한 가치를 만드는 비영리 연구기관을 표방했다. 연구자가 논문과 글, 코드를 공개하도록 장려하고 특허도 세계와 공유하겠다고 약속했다. 이후 [헌장](https://openai.com/charter/)에서도 AGI의 혜택이 널리 분배되어야 하며, 사회가 AGI에 이르는 데 필요한 공공재를 제공하겠다는 원칙을 남겼다.
        ...
        결국 NVIDIA가 경계하는 것은, 통제가 **NVIDIA가 들어설 수 없는 별도의 생태계를 키우는 상황**으로 읽힌다.
      ...
      en: >-
        Top to bottom represents the flow of dates; dotted lines show
        connections confirmed in public records or directly linked by content.
        Events without dotted lines occurred around the same time, but no direct
        ...
        **Distillation** is a technique where outputs from an already trained model serve as a teacher to train another. It is widely used in model development, evaluation, and compression. However, the US government and certain model companies contend that when executed as covert, large-scale extraction against a competitor's commercial model, it should be viewed not as a standard training practice, but as theft of intellectual property.
        ...
        In truth, the table above alone makes it difficult to explain OpenAI's position. Looking only at its current core business, OpenAI is a company that sells closed models and APIs. Given that the growth of open weight makes it harder to monopolize price and access, OpenAI seems closer to the very entity this letter seeks to keep in check.  


        Yet OpenAI's starting point was different from its current business structure.  


        In its 2015 founding statement, OpenAI presented itself as a non-profit research institution dedicated to creating value for everyone rather than shareholder profit. It encouraged researchers to publish papers, posts, and code, and promised to share patents with the world. In its subsequent charter, it retained the principle that the benefits of AGI should be widely distributed, and that it would provide the public goods necessary for society to reach AGI.  


        Reading this as a promise to release every model as open source would not be accurate. The charter itself already explicitly noted that traditional research sharing might decrease due to safety and security concerns. Still, it is clear that OpenAI's early identity rested on the principle that the fruits of powerful AI should not be concentrated in a single company, and that research findings and tools should be shared as widely as possible.  


        Traces of this remain. As recently as April, when releasing open-weight models such as the OpenAI Privacy Filter, OpenAI explained once more that putting AI into as many hands as possible is its mission. It also stated that choosing strictly between open models and closed models is a false dichotomy, and that both are needed.  


        Thus, OpenAI's signature is less a sudden pivot than a choice to maintain both its current frontier closed-model business and the open principles it proclaimed at its inception. Even if its most advanced models are sold in a controlled form, it does not want the layer of models beneath them—models anyone can download, modify, and run—to disappear.
        ...
        **Hugging Face ultimately completed the analysis by running GLM-5.2, released by China's Z.ai, on its own infrastructure**. As a result, credentials contained within the attacker's data and logs were not transmitted to external model providers' servers.
        ...
        And here one last question remains.


        **Why did this letter not simply speak of "open weight," but insist on attaching "American AI leadership" to its title?**  


        The most direct reason is that this text is a letter written to persuade US policymakers. Thus, the letter translates open weight into the language of American prosperity, technological sovereignty, and national competitiveness.  


        Yet it is not mere rhetoric. The letter argues that American AI leadership is not determined by a single superior frontier model, but by how widely AI spreads through society. From this perspective, open weight is not a near-free substitute, but a distribution network that draws individuals and companies into a specific nation's technological ecosystem. Depending on which country's model is adopted more widely, that nation's chips, cloud infrastructure, development tools, and technical standards spread alongside it.  


        **Now that the performance and pricing of Chinese open-weight models are rapidly catching up to, and in some cases surpassing, those of the US, this logic has grown more urgent.** If the US blocks Chinese models without building a strong open-model ecosystem of its own, American companies may become more deeply dependent on a small number of expensive APIs. Meanwhile, individuals and companies outside the US will continue to build products and tools on top of Chinese models. This means regulation, rather than briefly protecting American closed-model companies, could push the global open ecosystem itself toward China.  


        So in this letter, "American leadership" is not the opposite of openness. At least within the logic of the signatories, it is an argument that the US must go beyond not banning open weight to directly supplying and spreading it, if it is to maintain a global AI ecosystem centered on the US. It is in this same context that OpenAI, referring to its own open-weight model in 2025, used the expression "an American-led rail."  


        Yet this expression also reveals the letter's limits.  


        Is what they seek to defend openness itself, without borders? **Or is it openness only so long as America leads it?**  


        The letter does not mention China directly. But the impetus for the letter was the debate over restrictions on Chinese models, and in the representative case NVIDIA presented three days later, an American closed model was used for attack while a Chinese open-weight model was used for defense.  


        If the most powerful open-weight model comes from China next time as well, will these companies still defend American users' right to access it?
---
