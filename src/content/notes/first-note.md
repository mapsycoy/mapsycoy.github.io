...
date: 2026-08-15
titleEn: An Introduction to HCI Research Data and Statistics - Starting with
  Artwork Dwell Time
updated: 2026-08-15
status: settled
tags:
...
| 시각적 두드러짐(주의 포획)    | 거대한 미디어아트, 움직이는 조형물, 벽면을 채운 대형 회화 | 크기·움직임·대비, 시야에서 차지하는 비율 |
...
야외의 거대한 미디어아트, 움직이는 조형물, 층고가 높은 실내 한쪽 벽을 채운 대형 회화는 관람객이 해당 작가나 주제를 미리 선호하지 않았더라도 먼저 눈에 들어오거나 압도감을 줄 수 있다. 이렇게 자극이 시선을 먼저 가져가는 현상을 주의 포획(attentional capture)이라 한다.
...
같은 관람객이 다른 작품을 보면 행이 늘어난다. 만약 V02가 이후 작품 B를 DP 2에서 보았다면 여섯 번째 행이 생긴다. 이렇게 한 사람이 여러 행을 만드는 구조가 6장에서 다룰 반복측정의 출발점이다. 다만 3~7장에서는 계산을 단순화하기 위해 네 조건에 서로 다른 관람객이 참여했다고 가정하므로, 이 여섯 번째 행은 이후 계산에 포함하지 않는다.
...
평균 하나만 말하지 않는다.
**0초 비율, 중앙값, IQR[^1], 평균, SD와 실제 분포**를 함께 본다.
...
3장의 0, 2, 4, 29, 160초에 같은 규칙을 대보면 평균 39초, 표본 SD 68.7초이므로 구간이 −29.7초부터 107.7초가 된다. 음수 체류시간은 존재할 수 없다.
...
구조: `종속변수 ~ 고정효과들 + (랜덤효과 | 그룹변수)`

~ 오른쪽 앞부분은 고정효과로, 전체 자료를 통틀어 계수 하나씩을 추정한다. 괄호가 붙은 뒷부분이 랜덤효과다.

`체류시간 ~ DP + 유명세 + 제작시기 + 혼잡도 + 관람순서 + (1|관람객) + (1 + DP|작품)`

| 표기   | 읽는 법                         | 잡아내려는 것               |
| ---- | ---------------------- | --------------- |
| `(1\|관람객)` | 	관람객마다 절편(1)[^2]이 다름       | 어떤 사람은 전반적으로 빨리 보고, 어떤 사람은 전반적으로 오래 봄         |
| `(1 + DP\|작품)` | 작품마다 절편과 DP 기울기가 둘 다 다름       | 작품마다 기본 체류시간이 다를 뿐 아니라, DP를 바꿨을 때 늘어나는 정도도 다름      |
...
| p값   | 귀무가설과 통계모형의 가정이 맞을 때, 관측된 것과 같거나 더 극단적인 결과가 나올 확률은 얼마인가?[^3] | 보조적인 불일치 정보     |
...
5장에서 보았듯 이 평균 효과는 두 작품의 서로 다른 효과를 요약한 값이다. 여기서는 요약값의 불확실성만 다루지만, 실제 보고에서는 작품별 효과의 불확실성도 함께 제시해야 한다.

이 5초는 표본에서 관측된 평균 차이다. 그러나 다른 관람객을 다시 관찰해도 비슷한 차이가 나타날지는 알 수 없다. 표본에서 모집단으로 나아가려면 이 추정값이 얼마나 흔들릴 수 있는지 함께 살펴봐야 한다.

여기서 모집단은 우선 같은 두 작품과 같은 DP 조건을 앞으로 경험할 더 큰 관람객 집단을 뜻한다. 작품 A와 B만으로 모든 작품에 일반화할 수는 없다.
...
caption: 이 곡선은 참값이 5초라고 가정했을 때의 표집분포다. 실제 참값은 알 수 없으며, 구간의 중심이 참값이라는 뜻은 아니다.
```

이 구간은 0초를 포함한다. 따라서 이 작은 표본만으로는 DP 2의 평균 체류시간이 더 길다고 효과의 방향을 확정하기 어렵다. 그렇다고 DP 효과가 정확히 0초임이 입증된 것도 아니다. 추정 가능한 범위가 넓다는 뜻이다.

이 계산은 네 조건을 서로 독립적인 집단으로 취급한 입문용 정규근사다. 특히 작품 A · DP 1에는 160초라는 긴 관측값이 있어 표준오차와 신뢰구간이 크게 넓어졌다. 실제 연구에서는 반복 관측, 오른쪽으로 치우친 분포와 0초 관측을 반영한 모형을 사용해야 한다. 표본이 작을 때는 1.96 대신 t분포의 임계값을 쓰거나 부트스트랩을 사용한다.

* 95% 신뢰구간은 참값이 95% 확률로 이 안에 있다는 단순한 뜻이 아니다.
* 95%는 수학적 필연이 아니라 관례다. 90%나 99%도 쓸 수 있으며, 신뢰수준을 높이면 구간은 넓어진다. 연구 시작 전에 정하고 논문에 명시한다.
* 같은 절차를 반복하면 구간의 약 95%가 참값을 포함한다는 장기적 성질이다. 단, 이 95%는 모형 가정이 맞을 때 성립하는 명목값이다. 표본이 작거나 분포가 심하게 치우치면 실제 포함률은 95%에 못 미칠 수 있다.
...
| 추론  | “평균 차이의 추정값은 5초였지만, 정규근사 95% 신뢰구간은 약 [−25.4, 35.4]초로 넓었다.”                                      |
...
[^1]: IQR(사분위범위, interquartile range)은 데이터를 순서대로 놓았을 때 75% 지점인 제3사분위수(Q3)에서 25% 지점인 제1사분위수(Q1)를 뺀 값이다. 가운데 50%의 퍼짐을 나타내며, 극단값의 영향을 비교적 덜 받는다.
[^2]: (예)'체류시간 = 기본 체류시간 + (DP를 바꿔서 늘어난 초)' 여기서 기본 체류시간이 절편
[^3]: 쉽게 말해 "DP 효과가 0이라고 가정했을 때, 지금 정도의 차이가 우연히 나올 확률"이다.
