...
아래 3장과 5장에서는 위의 작품 A · DP 1 다섯 행만 사용한다.
...
표본 SD는 다섯 명을 더 큰 모집단에서 뽑은 표본으로 보고 모집단의 퍼짐을 추정하므로 분모에 n−1을 쓴다.
...
그래서 상호작용이 뚜렷할 때 주효과만 보고하면 오해를 부른다. DP의 주효과 5초는 두 작품 어디에서도 관측되지 않은 값이다. 앞서 평균 39초를 실제로 본 관람객이 한 명도 없었던 것과 같은 문제다. 상호작용이 있으면 주효과 대신 조건별 효과를 각각 보고한다.

두 줄의 평균은 똑같이 39초다. 그런데 표본 SD는 약 68.7초와 8.7초로 거의 8배 차이가 난다. 평균만 보면 두 상황이 같아 보이지만, 실제로는 전혀 다른 관람이 벌어지고 있다. 아래 점들은 그 차이를 숫자 그대로 보여준다.
...
DP 효과를 5초, SE를 1.5초로 추정했다고 하자.
정규근사 95% 신뢰구간은 약 2–8초다.
...
| 층 | 결과 문장 예시 |
| --- | --- |
| 관찰 | “표본에서 DP 2의 평균 체류시간은 DP 1보다 5초 길었다.” |
| 추론 | “추정 차이는 5초였고, 95% 신뢰구간은 2–8초였다.” |
