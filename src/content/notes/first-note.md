...
작품 앞 체류시간 사례를 따라 데이터와 통계의 기본 순서를 익혀보자.

---

## 질문

- 쓱 보고 지나간 사람은 `0초`인가, 데이터에서 빼야 하는가?
...
그럼에도 전공과 작업을 거치며 전시 공간은 가장 자주 접해온 장면이고, 좋아해서라기보다 구체적으로 떠올리고 질문할 수 있다는 점에서 내게 가장 익숙한 사례다.
대부분의 작품은 쓱 보고 넘어가므로 기본 체류시간도 짧은 편이다.
하지만 좋아하는 작가인 빈센트 반 고흐의 작품, 지브리처럼 관심 있는 주제의 기획전, 친한 친구의 전시라면 더 오래 머문다.

체류시간에 영향을 줄 수 있는 요인은 개인의 관심뿐 아니라 주의와 상황에도 걸쳐 있다.
...
| 지각적 현저성·주의 포획 | 거대한 미디어아트, 움직이는 조형물, 벽면을 채운 대형 회화 | 크기·움직임·대비, 시야에서 차지하는 비율 |
| 사회적 단서·군중효과 | 사람들이 몰린 작품을 따라 보거나 중요하다고 여김 | 주변 인원, 대기열, 집단의 체류와 행동 |

따라서 체류시간이 길어졌다는 결과만 보고 “몰입이 높았다”고 하나로 묶을 수 없다.
개인의 관심인지, 외부 자극이 포착한 주의인지, 주변 사람에게서 얻은 사회적 단서인지 연구 전에 구분하고 별도 변수로 측정해야 한다.
사람의 시선은 움직임, 큰 크기, 강한 대비처럼 주변에서 두드러진 자극에 빠르게 포착될 수 있다.
야외의 거대한 미디어아트, 움직이는 조형물, 층고가 높은 실내 한쪽 벽을 채운 대형 회화는 관람객이 해당 작가나 주제를 미리 선호하지 않았더라도 먼저 눈에 들어오거나 압도감을 줄 수 있다.
다만 “크면 누구나 오래 본다”는 법칙은 아니다.
실제 반응은 관람 거리, 시야각, 주변 자극, 혼잡도와 개인차에 따라 달라진다.

이때 **시선이 먼저 끌린 것**과 **그 대상을 좋아하거나 알고 싶어 하는 관심**도 구분해야 한다.
주의 포획은 멈춤 여부를 바꿀 수 있고, 작가 선호나 주제 관심은 멈춘 뒤의 체류시간을 바꿀 수 있다.
따라서 작품의 물리적 크기, 움직임, 밝기·대비와 시야에서 차지하는 비율도 함께 기록할 가치가 있다.
다른 관람객의 행동도 하나의 사회적 단서가 된다.
특정 작품에 사람이 몰리면 “볼 만한 이유가 있나 보다”라고 판단해 멈추거나 대기하면서 체류시간이 길어질 수 있다.
반대로 혼잡해서 작품이 잘 보이지 않거나 불편하면 빨리 떠날 수도 있다.
더구나 사람들이 오래 머물러 군중이 생긴 것인지, 군중을 보고 새 관람객이 오래 머문 것인지 관찰 자료만으로는 방향을 구분하기 어렵다.
그래서 단순 인원수뿐 아니라 도착 시점의 주변 인원, 대기 여부, 작품의 가시성, 앞선 관람객의 행동을 시간 순서와 함께 기록하는 편이 좋다.
같은 사람도 작품과 상황에 따라 달라지므로 개인의 기본 성향과 특정 전시에 대한 관심을 함께 기록하는 편이 좋다.
...
`0초`는 관찰한 행동이고 `NA`는 측정하지 못해 모르는 값이다.
멈춘 사람만 남기면 체류시간이 실제보다 길어질 수 있다.
긴 값도 무조건 삭제하지 말고 오류인지 실제 사례인지 확인한다.

---

## 2. 한 행은 무엇인가

**관람객 한 명이 작품 하나를 한 번 마주친 사건**을 한 행으로 둔다.
...
`nearby_people`는 관람이 시작된 시점에 미리 정한 작품 주변 구역 안에 있던 **다른 관람객 수**다. 기준 관람객 본인은 제외한다.
이 다섯 행의 `time_sec` 값은 3장에서 분포와 평균을 살펴볼 때 그대로 사용한다.

| 용어 | 질문 | 예 |
| --- | --- | --- |
| 관측 단위 | 무엇 하나를 사례로 세는가? | 한 사람×한 작품×한 번의 만남 |
| 관측 | 실제로 기록된 한 사례는? | V04가 B를 DP 2에서 29초 봄 |
...
이처럼 관측값 대부분은 작은 값 근처에 모여 있는데 소수의 큰 값이 오른쪽으로 길게 이어지는 모양을 **오른쪽으로 치우친 분포**라고 한다.
여기서 **꼬리**는 값이 드문 쪽으로 분포가 가늘고 길게 이어지는 부분을 가리킨다.
시간이 오른쪽으로 흐른다는 뜻도, 관람객 한 명의 행동이 꼬리처럼 변한다는 뜻도 아니다.

체류시간에서 이런 모양이 생길 수 있는 이유도 어렵지 않다.
많은 사람은 작품을 보지 않고 지나가거나 잠깐만 멈추지만, 일부는 캡션을 읽고 대화하거나 영상을 끝까지 보면서 훨씬 오래 머물 수 있다.
체류시간은 0보다 작아질 수 없지만 오래 머무는 쪽에는 이론적으로 같은 정도의 단단한 상한이 없기 때문에 긴 값이 오른쪽에 생기기 쉽다.
다만 모든 전시에서 반드시 이런 분포가 나타나는 것은 아니므로 실제 원자료를 먼저 확인해야 한다.
...
위의 매끈한 곡선은 **개념도**다.
다섯 명에게서 실제로 매끈한 분포를 관측한 것처럼 제시하면 안 된다.
실제 연구에서는 원자료 점, 히스토그램 또는 상자그림을 함께 사용한다.

---

## 4. 표준편차

이번 값은 `29, 33, 38, 44, 51초`다.
...
1. 각 값에서 평균을 뺀다.
2. 편차를 제곱한다.
3. 편차 제곱합을 목적에 따라 `n` 또는 `n−1`로 나눈 뒤 제곱근을 취한다.

평균 39초에서 각 값을 뺀 편차는 `−10, −6, −1, 5, 12`이고, 편차 제곱합은 `100 + 36 + 1 + 25 + 144 = 306`이다.
...
모집단 SD는 이 다섯 명 자체의 퍼짐을 기술할 때 분모에 `n`을 쓴다.
표본 SD는 다섯 명을 더 큰 모집단에서 뽑은 표본으로 보고 모집단의 퍼짐을 추정하므로 분모에 `n−1`을 쓴다.
...
mark: 30.3=−1 SD · 30.3, 47.7=+1 SD · 47.7
data: 29, 33, 38, 44, 51
xlabel: 작품 앞 체류시간(초)
...
이제 3번과 4번의 관람값을 버리지 않고 같은 연구의 다음 단계로 가져오자. 두 묶음은 같은 작품 A를 서로 다른 DP에서 본 관람객 5명의 기록이라고 가정한다.
...
앞에서 본 바로 그 값들이다. A의 평균은 두 DP에서 모두 `39초`지만 관람 양상은 전혀 다르다. DP 1에서는 세 사람이 짧게 보고 한 사람이 매우 오래 머문 반면, DP 2에서는 다섯 사람이 평균 주변에 비교적 가깝게 모였다.

**왜 5가 아니라 4로 나눌까: 작품 A · DP 1의 임시 기준선**

우리가 알고 싶은 기준은 앞으로 이 전시를 볼 더 큰 관람객 집단의 실제 평균이다. 하지만 지금 손에 있는 것은 작품 A를 DP 1에서 본 다섯 명의 기록뿐이다. 실제 평균을 모르므로 다섯 기록의 평균인 `39초`를 **임시 기준선**으로 삼는다.
...
`39초`는 임의로 고른 숫자가 아니다. 같은 다섯 기록에서 평균과의 거리제곱합이 가장 작아지는 위치다.

| 임시 기준선 | `0, 2, 4, 29, 160초`와의 거리제곱합 |
| ---: | ---: |
| 30초 | 19,261 |
| 35초 | 18,936 |
...
같은 기록에 가장 잘 맞도록 기준선을 정했으니, 그 선에서 잰 퍼짐은 더 큰 관람객 집단의 퍼짐을 작게 추정하는 경향이 있다. 이를 보정하기 위해 거리제곱합을 `5`가 아니라 `5−1`, 즉 `4`로 나눈다.

| 계산 | 결과 |
| --- | ---: |
| `18,856 ÷ 5`의 제곱근 | 약 61.4초 |
| `18,856 ÷ 4`의 제곱근 | 약 68.7초 |

왜 정확히 `4`일까? 다섯 기록으로 평균이라는 기준선 하나를 정하는 데 정보 한 몫을 사용했기 때문이다. 평균이 정해지면 다섯 편차의 합은 반드시 0이어서, 네 편차를 알면 마지막 편차는 자동으로 정해진다. 이를 **자유도가 4**라고 한다. 관람객 한 명을 버린다는 뜻은 아니다. 더 엄밀하게는 같은 방식으로 표본을 반복해서 뽑을 때 `n−1`로 나눈 **분산**이 모집단 분산을 평균적으로 정확히 추정하도록 하는 보정이다.

A · DP 1의 표본 SD는 이렇게 약 `68.7초`가 된다. A · DP 2도 같은 순서로 계산한다. 평균과의 차이를 제곱해 더한 값 `306`을 `4`로 나누고 제곱근을 구하면 약 `8.7초`다.

작품과 DP를 함께 비교하려면 나머지 두 칸이 더 필요하다. 작품 B를 본 관람객 5명의 가상 기록만 다음처럼 보탠다. 이것은 새 예시가 아니라 같은 `작품×DP` 표의 빈칸을 채우는 자료다.
...
- **작품의 주효과:** A의 전체 평균은 `39초`, B는 `19초`다. 이 자료에서는 A가 평균 20초 길다.
- **DP의 주효과:** DP 2의 전체 평균은 `31.5초`, DP 1은 `26.5초`다. 이 자료에서는 DP 2가 평균 5초 길다.
- **작품×DP 상호작용:** A에서는 DP가 바뀌어도 평균 차이가 `0초`지만, B에서는 `+10초`다.
- **차이의 차이:** `(39−39)−(24−14)=−10초`다. 부호는 비교 순서를 나타내고, 두 DP 효과가 서로 다른 크기는 10초다.

상호작용은 한 요인의 효과가 다른 요인에 따라 달라진다는 뜻이다. 여기서는 “DP 2가 항상 5초 늘린다”가 아니라 작품 A에서는 평균이 그대로이고 작품 B에서는 10초 늘어난다.

두 줄의 평균은 똑같이 `39초`다. 그런데 표본 SD는 약 `68.7초`와 `8.7초`로 거의 8배 차이가 난다. 평균만 보면 두 상황이 같아 보이지만, 실제로는 전혀 다른 관람이 벌어지고 있다. 아래 점들은 그 차이를 숫자 그대로 보여준다.
...
mark: 39=공통 평균
xlabel: 체류시간(초)
caption: 위 표의 작품 A: 두 조건 모두 평균 39초지만 표본 SD는 약 68.7초와 8.7초다
```

이 그림은 두 SD의 폭을 같은 축에서 비교하는 **모식도**다. A · DP 1의 실제 다섯 값은 오른쪽으로 심하게 치우쳤으므로 정규분포라고 볼 수 없다. 실제 모양은 3번의 점과 오른쪽 꼬리 그림에서 확인하고, 여기서는 같은 평균에도 퍼짐이 크게 다를 수 있다는 점만 읽는다.
...
DP 효과를 `20초`, SE를 `3초`로 추정했다고 하자.
정규근사 95% 신뢰구간은 약 `14–26초`다.
...
shade: between 14 26
mark: 14, 26
xlabel: 반복 표집에서 얻을 수 있는 DP 효과 추정값(초)
caption: 반복 표본에서 추정값이 흔들리는 모습을 단순화한 개념도
```
...
4. **측정 규칙을 고정한다.** 구역, 시작·종료, 0초, 재방문, `NA`를 정의한다.
5. **설계를 교차한다.** 여러 작품을 여러 DP에 둔다.
6. **맥락을 기록한다.** 혼잡도, 조명, 캡션, 동행, 순서를 남긴다.
7. **원자료를 점검한다.** 오류와 결측 사유를 확인한다.
8. **분포부터 그린다.** 0초 비율, 중앙값, IQR, 평균, SD를 본다.
9. **설계에 맞는 모형을 쓴다.** 관람객과 작품 반복을 반영한다.
10. **허용된 범위로 해석한다.** 효과크기와 불확실성을 보고한다.

| 층 | 결과 문장 예시 |
| --- | --- |
| 관찰 | “표본에서 DP 2의 평균 체류시간은 DP 1보다 20초 길었다.” |
| 추론 | “추정 차이는 20초였고, 95% 신뢰구간은 14–26초였다.” |
| 해석 | “DP가 관람 행동과 관련될 가능성을 지지하지만, 체류시간만으로 몰입이나 작품성을 단정할 수 없다.” |
